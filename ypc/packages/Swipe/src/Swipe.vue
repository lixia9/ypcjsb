<template>
  <div class="ypc-swipe">
    <div class="ypc-swipe-items-wrap" ref="wrap">
      <slot></slot>
    </div>
    <div class="ypc-swipe-indicator-wrap" :class="showTitle ? 'ypc-swipe-indicator-wrap-background' : ''" v-if="showIndicators">
      <div v-if="showTitle" class="ypc-swipe-item-title">{{title}}</div>
      <ul :class="[
         indicatorsPosition === 'right' ? 'ypc-swipe-indicators-right' : '',
         true ? 'ypc-swipe-indicators' : ''
        ]" v-if="indicatorsType === 'dot'">
        <li v-for="(page, $index) in pages"
            :class="[
           $index === index ? 'current' : '',
           true ? 'ypc-swipe-indicator' : '']"></li>
      </ul>
      <div v-if="indicatorsType === 'number'" class="ypc-swipe-number-indicator">{{index+1}} / {{pages.length}}</div>
    </div>
  </div>
</template>
...
.ypc-swipe-items-wrap > .ypc-swipe-item {
  position: absolute;
...
.ypc-swipe-items-wrap > .ypc-swipe-item.current {
  display: block;
  transform: none;
}

.ypc-swipe-indicator-wrap {
  position: absolute;
  bottom: 0;
  width: 100%;
  height: 0.9333333333333333rem;
}
.ypc-swipe-indicator-wrap-background {
  background: rgba(0,0,0,.2)
}

.ypc-swipe-item-title {
  line-height: 0.9333333333333333rem;
  font-size: 0.4rem;
  text-indent: 1em;
}

.ypc-swipe-indicators {
  position: absolute;
  bottom: 0.24rem;
  left: 50%;
...
.ypc-swipe-indicator:first-child {
  margin-left: 0;
}
.ypc-swipe-indicator:last-child {
  margin-right: 0;
}
.ypc-swipe-indicator.current {
  opacity: 1;
}

.ypc-swipe-indicators-right {
  position: absolute;
  left: auto;
...
.ypc-swipe-vertical-indicators {
  position: absolute;
  right: 0.6666666666666666rem;
  top: 50%;
  transform: translateY(-50%);
}
.ypc-swipe-vertical-indicator {
  display: block;
  ...
  border-radius: 100%;
  margin: 0 0 0.5333333333333333rem 0;
  cursor: pointer;
...
.ypc-swipe-vertical-indicator:after,
.ypc-swipe-vertical-indicator:before {
  content: '';
  display: inline-block;
  width: 0.41333333333333333rem;
...
.ypc-swipe-vertical-indicator:before {
  ...
  opacity: .3
;}
.ypc-swipe-vertical-indicator:hover,
.ypc-swipe-vertical-indicator.current {
  border-color: #fff;
  background-color: #258fb8;
}
.ypc-swipe-vertical-indicator.current:before,
.ypc-swipe-vertical-indicator.current:after {
  border-color: #258fb8;
